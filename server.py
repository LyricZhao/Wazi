    ...
    async def run_server(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as src_socket:
            ...
            while True:
                ...
                src_to_dst_task = self.loop.create_task(self.recv_and_send("src2dst", self.loop, src_conn, dst_socket, cipher_func=self.decode))
                dst_to_src_task = self.loop.create_task(self.recv_and_send("dst2src", self.loop, dst_socket, src_conn, cipher_func=self.encode))
