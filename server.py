...
from cipher import RSACipher, EmptyCipher
...
class Server():
    def __init__(self, loop, host, port, pub_key_path, pri_key_path, passwd_path):
        ...
        with open(passwd_path, "r") as f:
            self.passwd = f.readlines()[0].strip()
    ...
    async def handle(self, src_conn, src_addr):
        ...
        if src_ip not in self.ip_key_pool:
            ...
            h = data[:4]
            if h not in self.hash.hash_tuple(self.passwd):
                data = self.rsa_cipher.decode(data[4:])
                
                if len(data) != 128 + 16:
                    src_conn.close()
                
                passwd = data[:128].decode()
                if passwd != self.passwd:
                    print(src_ip, "wrong passward, close connection")
                    src_conn.close()
                    return
                R = data[128:]
                self.ip_key_pool[src_ip] = R
        else:
            aes_cipher = EmptyCipher() # NOTE get from pool
            ...
            src_to_dst_task = self.loop.create_task(self.recv_and_send(
                "src2dst", self.loop, src_conn, dst_socket, cipher_func=aes_cipher.decode))
            dst_to_src_task = self.loop.create_task(self.recv_and_send(
                "dst2src", self.loop, dst_socket, src_conn, cipher_func=aes_cipher.encode))
